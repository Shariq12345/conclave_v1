...
Output: research_paper/figures/fig_fedguard_architecture.pdf (300+ DPI PDF)
        research_paper/figures/fig_fedguard_architecture.png (PNG Preview)
"""
...
OUTPUT_PDF = "research_paper/figures/fig_fedguard_architecture.pdf"
OUTPUT_PNG = "research_paper/figures/fig_fedguard_architecture.png"
...
def create_architecture_diagram():
    # 7.1 inches wide (full double-column width in IEEEtran), 5.8 inches height
    fig, ax = plt.subplots(figsize=(7.1, 5.8), dpi=300)
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.axis('off')

    # Premium Color Palette - Modern IEEE Security Theme
    # Layer 1: Participant & Root-of-Trust (Teal Theme)
    C_L1_CARD_BG = "#f0fdf4"
    C_L1_BORDER  = "#0d9488"
    C_L1_TITLE   = "#064e3b"
    C_L1_BOX_BG  = "#ccfbf1"

    # Layer 2: Governance Control Plane (Indigo Theme)
    C_L2_CARD_BG = "#eef2ff"
    C_L2_BORDER  = "#4f46e5"
    C_L2_TITLE   = "#1e1b4b"
    C_L2_BOX_BG  = "#e0e7ff"

    # Layer 3: Crypto & Aggregation Engine (Slate Theme)
    C_L3_CARD_BG = "#f8fafc"
    C_L3_BORDER  = "#334155"
    C_L3_TITLE   = "#0f172a"
    C_L3_BOX_BG  = "#e2e8f0"

    # Layer 4: Audit Ledger & Storage (Rose/Crimson Theme)
    C_L4_CARD_BG = "#fff1f2"
    C_L4_BORDER  = "#e11d48"
    C_L4_TITLE   = "#881337"
    C_L4_BOX_BG  = "#ffe4e6"

    # Helper function to draw rounded outer layer cards
    def draw_layer_card(x, y, w, h, bg, border, title="", title_color="#000000", lw=1.2, r=1.5):
    ...
    # Helper function to draw inner functional component boxes
    def draw_component_box(x, y, w, h, bg, border, text_lines, text_color="#000000", lw=1.0, align='left', r=1.0):
    ...
    # Helper function to draw sleek connecting arrows with floating text badges
    def draw_connection_arrow(x1, y1, x2, y2, color="#475569", style='->', lw=1.1, text="", text_pos=None):
        ax.annotate(
    ...
    draw_layer_card(1.5, 74.5, 97, 21.5, C_L1_CARD_BG, C_L1_BORDER, "1. Participant Layer: Multi-Institutional Edge Nodes & Hardware Root-of-Trust", C_L1_TITLE)

    # 3 Client Nodes Subboxes (y = 75.8, h = 15.2 -> top = 91.0)
    draw_component_box(3.5, 75.8, 29, 15.2, C_L1_BOX_BG, C_L1_BORDER, [
    ...
    ], C_L1_TITLE, align='left')

    draw_component_box(35.5, 75.8, 29, 15.2, C_L1_BOX_BG, C_L1_BORDER, [
    ...
    draw_component_box(67.5, 75.8, 29, 15.2, C_L1_BOX_BG, C_L1_BORDER, [
    ...
    draw_connection_arrow(18, 75.8, 18, 69.5, C_L1_BORDER, '->', 1.1, "mTLS / Quote", (18, 72.65))
    draw_connection_arrow(50, 75.8, 50, 69.5, C_L1_BORDER, '->', 1.1, "mTLS / Quote", (50, 72.65))
    draw_connection_arrow(82, 75.8, 82, 69.5, C_L1_BORDER, '->', 1.1, "mTLS / Fallback", (82, 72.65))


    # 2. LAYER 2: GOVERNANCE CONTROL PLANE (y = 42.5 to 69.5, h = 27.0)
    draw_layer_card(1.5, 42.5, 97, 27.0, C_L2_CARD_BG, C_L2_BORDER, "2. FedGuard Decoupled Governance Control Plane Engine", C_L2_TITLE)

    # Subbox 2A: ABAC/RBAC Policy Verification Engine
    draw_component_box(3.5, 43.8, 29, 20.8, C_L2_BOX_BG, C_L2_BORDER, [
    ...
    ], C_L2_TITLE, align='left')

    # Subbox 2B: Governance Mode & Threshold SecAgg Handler
    draw_component_box(35.5, 43.8, 29, 20.8, C_L2_BOX_BG, C_L2_BORDER, [
    ...
    draw_component_box(67.5, 43.8, 29, 20.8, C_L2_BOX_BG, C_L2_BORDER, [
    ...
    draw_connection_arrow(32.5, 54.2, 35.5, 54.2, C_L2_BORDER, '->', 1.0)
    draw_connection_arrow(64.5, 54.2, 67.5, 54.2, C_L2_BORDER, '->', 1.0)

    # Vertical Inter-Layer Arrows Layer 2 -> Layer 3
    draw_connection_arrow(18, 43.8, 18, 37.5, C_L2_BORDER, '->', 1.1, "Policy Authorization", (18, 40.65))
    draw_connection_arrow(50, 43.8, 50, 37.5, C_L2_BORDER, '->', 1.1, "SecAgg / Mask Rules", (50, 40.65))
    draw_connection_arrow(82, 43.8, 82, 37.5, C_L2_BORDER, '->', 1.1, "DP Noise / Revocation", (82, 40.65))


    # 3. LAYER 3: CRYPTOGRAPHIC & AGGREGATION EXECUTION ENGINE (y = 21.0 to 37.5, h = 16.5)
    draw_layer_card(1.5, 21.0, 97, 16.5, C_L3_CARD_BG, C_L3_BORDER, "3. Cryptographic Execution & Robust Aggregation Layer (Flower Integration)", C_L3_TITLE)

    draw_component_box(3.5, 22.2, 29, 11.2, C_L3_BOX_BG, C_L3_BORDER, [
        ("Modular Tensor Quantizer", 7.8, 'bold'),
        ("• Scale γ = 2^16, Modulus R = 2^32-1", 6.3, 'normal'),
        ("• y_u = x_u + Σ PRG(s) (mod R)", 6.3, 'bold')
    ], C_L3_TITLE, align='left')

    draw_component_box(35.5, 22.2, 29, 11.2, C_L3_BOX_BG, C_L3_BORDER, [
        ("Byzantine Robust Aggregators", 7.8, 'bold'),
        ("• Trimmed Mean (β=0.2), Median", 6.8, 'normal'),
        ("• Krum Coordinate Distance Filter", 6.8, 'normal')
    ], C_L3_TITLE, align='left')

    draw_component_box(67.5, 22.2, 29, 11.2, C_L3_BOX_BG, C_L3_BORDER, [
    ...
    draw_connection_arrow(32.5, 27.8, 35.5, 27.8, C_L3_BORDER, '->', 1.0)
    draw_connection_arrow(64.5, 27.8, 67.5, 27.8, C_L3_BORDER, '->', 1.0)


    # 4. LAYER 4: AUDIT LEDGER & MACHINE UNLEARNING (y = 1.0 to 17.0, h = 16.0)
    draw_layer_card(1.5, 1.0, 97, 16.0, C_L4_CARD_BG, C_L4_BORDER, "4. Immutable Storage & Verification Layer", C_L4_TITLE)

    draw_component_box(3.5, 2.4, 45, 10.5, C_L4_BOX_BG, C_L4_BORDER, [
    ...
    ], C_L4_TITLE, align='center')

    draw_component_box(51.5, 2.4, 45, 10.5, C_L4_BOX_BG, C_L4_BORDER, [
    ...
    draw_connection_arrow(26, 22.2, 26, 12.9, C_L4_BORDER, '->', 1.1, "Audit Event Logging", (26, 17.55))
    draw_connection_arrow(74, 22.2, 74, 12.9, C_L4_BORDER, '->', 1.1, "Purify Checkpoints", (74, 17.55))
