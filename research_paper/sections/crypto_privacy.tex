...
To prevent a semi-honest aggregator from inspecting individual client update tensors $x_u \in \mathbb{R}^d$, FedGuard integrates a threshold Secure Aggregation protocol based on pairwise pseudo-random masking and key sharing~\cite{bonawitz2017practical, so2021lightsecagg}.
...
Before transmission, each client bounds the influence of any single record by clipping local update tensors to a maximum $L_2$ norm threshold $C = 1.0$:
\begin{equation}
...
\end{equation}
To express privacy guarantees in standard $(\epsilon, \delta)$-DP, we convert RDP bounds via the optimal dual optimization at $\delta = 10^{-5}$~\cite{balle2020hypothesis}:
\begin{equation}
...
\begin{enumerate}
    ...
    \end{equation}
    \item \textbf{Krum Aggregation:} For each client $i \in \mathcal{U}$, let $S_i \subset \mathcal{U} \setminus \{i\}$ denote the subset of $N - f - 2$ closest updates to $x_i$ in Euclidean $L_2$ distance, where $f$ is the upper bound on Byzantine clients~\cite{blanchard2017machine}. Krum selects update vector $x_k$ where:
    \begin{equation}
