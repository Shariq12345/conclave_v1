...
\begin{equation}
    D_\alpha\big( \mathcal{M}(D) \parallel \mathcal{M}(D') \big) = \frac{1}{\alpha - 1} \ln \mathbb{E}_{x \sim \mathcal{M}(D')}\left[ \left( \frac{\mathcal{M}(D)(x)}{\mathcal{M}(D')(x)} \right)^\alpha \right]
\end{equation}
where $D_\alpha(\cdot \parallel \cdot) \le \epsilon_{\text{RDP}}(\alpha)$.
